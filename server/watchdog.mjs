// The watchdog — turn the execution list the server already fetches into
...
// over data fetched by n8n.mjs (GET-only) and writes nothing itself; the
// caller publishes the result through the existing notifications.json feed. The
// server's "no write path to n8n" invariant is untouched by design.
//
...
/**
 * The alert for "the server could not complete a poll at all".
 *
...
export function unreachableAlert(err, { baseUrl = '' } = {}) {
  ...
  return {
    ...
    message: `The po11y server could not complete a poll — ${detail}`,
    since: null,
  };
}
 ...
 *
 * Without this the server would re-announce every open alert on every poll —
...
// it. It used to have a second copy hard-coded in the hn-notify example
// workflow, pinned by a sibling test; that workflow wrote the feed as a file
// on a volume that no longer exists and has been deleted, so this is now the
// only copy there is.
