...
- **Architecture Map** (the **Architecture** tab): Displays an interactive map of your workflows, updated automatically every 10 minutes. Relationships are calculated deterministically; the per-workflow descriptions are LLM-generated by default (see the note above to keep them local).
...
services are a different model — they generally ask you to instrument each
workflow with success and failure callbacks — and are not compared here.
