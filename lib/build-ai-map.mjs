...
//
// Pure extraction of the "Build + publish ai-map.json" Code node from
// workflows/core/maps.json: the skeleton, digest/sig computation, publish
// policy, differential-annotation logic and fence-stripping JSON parse move
// over verbatim. The caller owns all I/O and time: it supplies the previous
// map (`prev`), the "Build now" form state (`forced`), the wall clock
// (`now`), the AI config booleans (`aiConfigured`/`model`) and the LLM
// transport (`llm`), and it stamps `generated_at` + writes the file. The
// original `console.log` for an unusable LLM reply becomes a `warnings`
// array on the returned summary.
...
export async function buildAiMap(workflows, {
    ...
    for (const n of wf.nodes || []) {
      ...
      if (kind) {
        ...
        if (kind === 'schedule') {
          ...
          const unit = { minutes: 'min', hours: 'h', seconds: 's', days: 'd' }[iv.field] || '';
  ...
  nodes.sort((a, b) => a.col - b.col || a.name.localeCompare(b.name) || a.id.localeCompare(b.id));
  ...
  const prevAge = prev ? now - new Date(prev.generated_at).getTime() : Infinity;
  const fresh = prevAge < 20 * 3600 * 1000;
  ...
  if (sameContent && !forced && (prevAnnotated || !aiConfigured)) {
    // Content-identical map — refresh the timestamp, spend no LLM tokens.
  ...
  if (aiConfigured) {
    ...
    if (!forced && prevAnnotated && prev.sigs) {
      ...
      for (const n of nodes) if (!target.includes(n) && prevSubs.get(n.id)) n.sub = prevSubs.get(n.id);
    }
