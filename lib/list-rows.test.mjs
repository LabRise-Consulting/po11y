...
import { normalizeRows, sortItems, groupByDay, parseDetail, RANGES, rangeCutoff, filterByRange,
  badgeValues, filterByBadges, windowComplete, dedupeById, pageUrl, walkPages } from './list-rows.mjs';
...
test('badgeValues: distinct, sorted, badge-less rows ignored', () => {
  const items = [
    { badge: 'webhook' }, { badge: 'sentry' }, { badge: null },
    { badge: 'webhook' }, { badge: 'grafana' },
  ];
  assert.deepEqual(badgeValues(items), ['grafana', 'sentry', 'webhook']);
  assert.deepEqual(badgeValues([{ badge: null }]), []);
});

test('filterByBadges: an empty selection means no filter', () => {
  const items = [{ badge: 'webhook' }, { badge: null }];
  assert.equal(filterByBadges(items, []).length, 2);
  assert.equal(filterByBadges(items, new Set()).length, 2);
  assert.equal(filterByBadges(items, null).length, 2);
});

test('filterByBadges: keeps only the selected sources, dropping badge-less rows', () => {
  const items = [
    { badge: 'sentry', title: 'a' },
    { badge: 'webhook', title: 'b' },
    { badge: 'grafana', title: 'c' },
    { badge: null, title: 'd' },
  ];
  assert.deepEqual(
    filterByBadges(items, ['sentry', 'webhook']).map((i) => i.title), ['a', 'b']);
  assert.deepEqual(filterByBadges(items, new Set(['webhook'])).map((i) => i.title), ['b']);
});

test('filterByBadges: leaves the input array untouched', () => {
  const items = [{ badge: 'webhook' }, { badge: 'sentry' }];
  filterByBadges(items, ['webhook']);
  assert.equal(items.length, 2);
});
...
test('pageUrl: appends limit, keeps existing query params', () => {
  const u = pageUrl('/n8n-table/data-tables/42/rows?sortBy=firstSeen:desc',
    { pageSize: 250, base: 'http://x' });
  assert.match(u, /limit=250/);
  assert.match(u, /sortBy=firstSeen%3Adesc|sortBy=firstSeen:desc/);
});

test('pageUrl: forceIdSort replaces a tied sort column', () => {
  // docs/configuration.md:287 — an offset walk over a tied column serves some
  // rows twice and never returns others. The MCP always walks by id.
  const u = pageUrl('/n8n-table/data-tables/42/rows?sortBy=firstSeen:desc',
    { base: 'http://x', forceIdSort: true });
  assert.match(u, /sortBy=id%3Adesc|sortBy=id:desc/);
  assert.ok(!u.includes('firstSeen'));
});

test('pageUrl: adds the cursor only from the second page on', () => {
  const first = pageUrl('/rows', { base: 'http://x' });
  const next = pageUrl('/rows', { base: 'http://x', cursor: 'abc' });
  assert.ok(!first.includes('cursor='));
  assert.match(next, /cursor=abc/);
});

test('walkPages: follows nextCursor and dedupes by id', async () => {
  const pages = [
    { data: [{ id: 1, title: 'a' }, { id: 2, title: 'b' }], nextCursor: 'c1' },
    { data: [{ id: 2, title: 'b' }, { id: 3, title: 'c' }], nextCursor: null },
  ];
  let n = 0;
  const out = await walkPages(async () => pages[n++], {
    endpoint: '/rows', mapping: { title: 'title' }, maxPages: 5,
    range: 'all', today: '2026-08-01', base: 'http://x',
  });
  assert.equal(out.rows.length, 3);
  assert.equal(out.truncated, false);
});

test('walkPages: reports truncated when the page budget is what stopped it', async () => {
  const out = await walkPages(async () => ({ data: [{ id: 1 }], nextCursor: 'more' }), {
    endpoint: '/rows', mapping: {}, maxPages: 2,
    range: 'all', today: '2026-08-01', base: 'http://x',
  });
  assert.equal(out.truncated, true);
});

test('walkPages: a static JSON feed carries no cursor and settles in one pass', async () => {
  let calls = 0;
  const out = await walkPages(async () => { calls++; return [{ id: 1 }, { id: 2 }]; }, {
    endpoint: '/static.json', mapping: {}, maxPages: 8,
    range: 'all', today: '2026-08-01', base: 'http://x',
  });
