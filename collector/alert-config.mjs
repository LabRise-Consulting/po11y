...
export function loadAlertConfig(env = process.env, log = console.error) {
  const num = (v, dflt, name) => {
    ...
    if (invalid) log(`collector: ${name}="${v}" is not a valid number — using ${dflt}`);
    return value;
  };
  ...
  if (path) {
    try { file = JSON.parse(readFileSync(path, 'utf8')); } catch (e) {
      log(`collector: ALERT_RULES_FILE unreadable (${e.message}) — using env only`);
    }
  }
  ...
  return {
    ...
    // exist instead of 404 on a fresh Mode B. ALERTS_ENABLED=false or a rules
